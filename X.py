...
def create_model(input_shape=(224, 224, 3), weights_path=None):
    # Xception 모델 생성
    ...
    for layer in base_model.layers[-20:]:  # Fine-tuning할 레이어 수 조정
        layer.trainable = True

    model = Sequential([
        base_model,
        GlobalAveragePooling2D(),
        Dense(512, activation='relu', kernel_regularizer=l2(0.001)),  # L2 정규화 강화
        Dropout(0.3),  # Dropout 비율 조정
        Dense(256, activation='relu', kernel_regularizer=l2(0.001)),  # L2 정규화 추가
...
# 모델 생성
model = create_model(weights_path='data/xception_weights_tf_dim_ordering_tf_kernels_notop.h5')
#############
...
#########
# 모델 저장
model.save('Xc_model.h5')from tensorflow.keras.models import load_model
