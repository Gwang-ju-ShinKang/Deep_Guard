...
# Xception 모델 생성
def create_xception(input_shape=(224, 224, 3), weights_path=None):  # weights_path 인자 추가
    base_model = Xception(weights=weights_path if weights_path else 'imagenet', include_top=False, input_shape=input_shape)
    base_model.trainable = False  # 기본 모델의 레이어를 고정
...
# EfficientNet 모델 생성
def create_efficientnet(input_shape=(224, 224, 3), weights_path=None):  # weights_path 인자 추가
    base_model = EfficientNetB0(weights=weights_path if weights_path else 'imagenet', include_top=False, input_shape=input_shape)
...
# 모델 생성
# 로컬 경로로 가중치 파일 지정
xception = create_xception(weights_path='data/xception_weights_tf_dim_ordering_tf_kernels_notop.h5')
efficientnet = create_efficientnet(weights_path='data/efficientnetb0_notop.h5')
